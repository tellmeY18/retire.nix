...
let
  ...
  # ════════════════════════════════════════════════
  rootDevice = "/dev/sda1"; # OLD root partition (before disko wipes it)
  ...
  kexecConfig = pkgs.nixos ({ config, pkgs, lib, ... }: {
    ...
    system.build.kexecTarball = lib.mkForce (pkgs.runCommand "nixos-kexec-installer-x86_64-linux.tar.gz"
      {
